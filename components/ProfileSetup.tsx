          ...
          {formData.role === UserRole.UNDEFINED ? (
            ...
            <form onSubmit={handleProfileSubmit} className="space-y-10 relative z-10">
              <div className="flex flex-col items-center gap-4">
                <div onClick={handleAvatarClick} className="relative w-20 h-20 rounded-full border-2 border-[#BF953F]/40 flex items-center justify-center cursor-pointer group hover:bg-[#BF953F]/10 transition-all overflow-hidden bg-black/40">
                  {formData.avatarUrl ? (
                    <img src={formData.avatarUrl} alt="Preview" className="w-full h-full object-cover" />
                  ) : (
                    <div className="text-center">
                      <Icons.Plus className="w-4 h-4 text-[#BF953F] mx-auto mb-1" />
                      <span className="text-[7px] font-black uppercase tracking-widest text-[#BF953F]">Logo</span>
                    </div>
                  )}
              ...
              <div className="space-y-10">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-3">
                    <label className="text-[8px] uppercase text-zinc-500 font-black tracking-[0.2em]">
                      {verificationStep === 'VERIFIED' ? 'VERIFIED ACCOUNT (LOCKED)' : 'BRAND NAME'}
                    </label>
                    <div className={`w-full bg-black border p-4 h-[65px] flex items-center transition-all ${verificationStep === 'VERIFIED' ? 'border-[#1DA1F2]/60 text-[#1DA1F2]' : 'border-white/10 focus-within:border-[#BF953F]'}`}>
                      {verificationStep === 'VERIFIED' ? (
                        ...
                        <input 
                          ...
                          className="w-full bg-transparent text-base font-black uppercase tracking-tighter outline-none text-white placeholder:text-zinc-800" 
                          value={formData.name} 
                  ...
                  <div className="space-y-3">
                    <label className="text-[8px] uppercase text-zinc-500 font-black tracking-[0.2em]">BROADCAST CLUSTER</label>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                  <div className="space-y-3">
                    <label className="text-[8px] uppercase text-zinc-500 font-black tracking-[0.2em]">IDENTITY PROOF (PRIMARY)</label>
                    
                    {verificationStep === 'IDLE' && (
                      <div className="flex gap-1 h-[65px]">
                        <input 
                          type="text" 
                          placeholder="@XHandle" 
                          className="flex-grow bg-black border border-white/10 p-4 text-sm font-black text-white outline-none focus:border-[#1DA1F2] placeholder:text-zinc-800" 
                          value={tempHandle}
                          onChange={e => setTempHandle(e.target.value)}
                        />
                        <button type="button" onClick={startVerification} className="bg-[#1DA1F2] text-black px-4 font-black uppercase text-[8px] tracking-widest hover:brightness-110 transition-all">Verify</button>
                      </div>
                    )}
                    ...
                    {verificationStep === 'CHALLENGE' && (
                      <div className="bg-black border border-[#1DA1F2]/20 p-5 space-y-4 animate-fadeIn">
                        <div className="space-y-1">
                           <p className="text-[8px] text-zinc-500 uppercase font-black tracking-[0.2em]">AUTHENTICATION PROTOCOL</p>
                           <p className="text-[8px] text-white/40 leading-relaxed uppercase">1. Post code to X. 2. Paste URL. Handle must match.</p>
                        </div>
                        <div className="space-y-4">
                          <a 
                            ...
                            className="block bg-[#1DA1F2]/10 p-3 border border-dashed border-[#1DA1F2]/40 text-center hover:bg-[#1DA1F2]/20 transition-all group"
                          >
                            <code className="text-[#1DA1F2] font-black text-base tracking-[0.1em] group-hover:brightness-125">{verifyCode}</code>
                            <p className="text-[7px] text-[#1DA1F2] mt-1 font-black">CLICK TO DRAFT TWEET</p>
                          </a>
                          <div className="space-y-2">
                             <input 
                               type="url" 
                               placeholder="Post URL"
                               className="w-full bg-black border border-white/10 p-3 text-[9px] text-white outline-none focus:border-[#1DA1F2] placeholder:text-zinc-800"
                               value={tweetUrl}
                               onChange={e => setTweetUrl(e.target.value)}
                             />
                             <button type="button" onClick={handleVerifyNow} className="w-full bg-white text-black py-3 font-black uppercase text-[9px] tracking-widest hover:bg-[#1DA1F2] hover:text-white transition-all">VALIDATE</button>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    {verificationStep === 'SCANNING' && (
                      <div className="w-full bg-black border border-[#1DA1F2]/20 p-6 flex flex-col items-center justify-center space-y-4 h-[65px] relative overflow-hidden">
                        <div className="absolute inset-0 bg-[#1DA1F2]/5 animate-pulse"></div>
                        <div className="w-3/4 h-0.5 bg-white/5 relative overflow-hidden rounded-full">
                           <div className="absolute inset-0 bg-[#1DA1F2] w-1/4 animate-[scanning_1.5s_infinite] shadow-[0_0_15px_rgba(29,161,242,0.8)]"></div>
                        </div>
                        <p className="text-[8px] text-[#1DA1F2] font-black uppercase tracking-[0.4em] z-10">SCRAPING X...</p>
                        <style>{`@keyframes scanning { 0% { left: -25% } 100% { left: 100% } }`}</style>
                      </div>
                    )}

                    {verificationStep === 'VERIFIED' && (
                      <div className="w-full bg-black border border-[#1DA1F2]/60 p-3 flex items-center justify-between group relative h-[65px] shadow-[0_0_20px_rgba(29,161,242,0.1)]">
                        <div className="flex items-center gap-3">
                           <div className="text-[#1DA1F2] drop-shadow-[0_0_12px_rgba(29,161,242,0.7)]">
                              <Icons.Check className="w-8 h-8" strokeWidth={5} />
                           </div>
                           <div className="space-y-0">
                             <p className="text-[7.5px] font-black uppercase tracking-[0.1em] text-[#1DA1F2]">VERIFIED TERMINAL</p>
                             <p className="text-lg md:text-xl font-black text-white leading-none tracking-tighter truncate max-w-[120px]">{formData.xHandle}</p>
                           </div>
                        </div>
                        <div className="flex flex-col items-end justify-between h-full">
                          <div className="absolute -top-[1px] -right-[1px] bg-[#1DA1F2] text-black px-3 py-1.5 font-black text-[8px] uppercase tracking-widest shadow-lg">
                            IDENTITY VERIFIED
                          </div>
                          <button 
                            type="button" 
                            onClick={() => { setVerificationStep('IDLE'); setFormData({...formData, isXVerified: false, xHandle: '', name: ''}); setTweetUrl(''); }} 
                            className="text-[8px] text-zinc-700 hover:text-white uppercase font-black tracking-[0.2em] transition-colors mt-auto"
                          >
                            RESET
                          </button>
                        </div>
                  ...
                  <div className="space-y-3">
                    <label className="text-[8px] uppercase text-zinc-500 font-black tracking-[0.2em]">CATEGORY / NICHE</label>
                    <div className="relative h-[65px]">
                      <select 
                        className="w-full h-full bg-black border border-white/5 px-4 text-xl font-black uppercase tracking-tighter focus:border-[#BF953F] outline-none text-white appearance-none cursor-pointer text-center" 
                ...
                <div className="space-y-3 pt-2">
                  <label className="text-[8px] uppercase text-zinc-500 font-black tracking-[0.2em]">PROJECT DIRECTIVE</label>
                  <textarea placeholder="Briefly describe your audience demographics..." className="w-full bg-black border border-white/10 p-5 min-h-[90px] text-[12px] font-medium focus:border-[#BF953F] outline-none text-zinc-400 transition-all resize-none leading-relaxed" value={formData.bio} onChange={e => setFormData({...formData, bio: e.target.value})} required />
                </div>
                
                <div className="flex justify-between items-center pt-8 border-t border-white/5">
                  <button type="button" onClick={() => setFormData({...profile, role: UserRole.UNDEFINED})} className="text-[8px] uppercase font-black tracking-[0.4em] text-zinc-600 hover:text-white transition-colors">BACK</button>
                  <button type="submit" className="bg-white text-black px-12 py-4 font-black uppercase text-[10px] tracking-[0.5em] hover:bg-[#BF953F] hover:text-white transition-all">SAVE IDENTITY</button>
                </div>
              </div>
            </form>
          )}
    ...
    <div className="max-w-6xl mx-auto space-y-12 py-8 animate-fadeIn pb-20">
      <section className="glass p-10 rounded-none border-white/20">
        <div className="flex flex-col md:flex-row items-center md:items-start gap-10">
          <div className="w-28 h-28 bg-black border-2 border-[#BF953F]/60 rounded-full flex items-center justify-center overflow-hidden shrink-0">
            {profile.avatarUrl ? (
              <img src={profile.avatarUrl} alt="Avatar" className="w-full h-full object-cover" />
            ) : (
              <span className="text-3xl font-black text-[#BF953F] opacity-40">{profile.name.slice(0,2).toUpperCase()}</span>
            )}
          </div>
              ...
              <div className="flex flex-col md:flex-row items-center justify-between border-b border-white/5 pb-8 gap-6">
                <div>
                  <div className="flex items-center justify-center md:justify-start gap-3">
                    <h2 className="text-4xl font-black uppercase tracking-tighter text-white leading-none">{profile.name}</h2>
                    {profile.isXVerified && (
                      <div className="text-[#1DA1F2] flex items-center" title="X Verified Identity">
                        <Icons.Check className="w-6 h-6 drop-shadow-[0_0_15px_rgba(29,161,242,0.6)]" />
                      </div>
                    )}
                  </div>
                  <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 mt-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(profile.platforms || []).map(p => (
                        <span key={p} className="text-[9px] bg-[#BF953F]/10 text-[#BF953F] px-4 py-1 font-black uppercase tracking-widest border border-[#BF953F]/20">{p}</span>
                      ))}
                    </div>
                  </div>
                </div>
                <button onClick={() => setIsEditing(true)} className="text-[9px] text-zinc-500 hover:text-[#BF953F] uppercase font-black tracking-[0.4em] border border-white/10 px-6 py-3 transition-all">Edit Terminal</button>
              </div>
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-12 py-4">
                <div className="space-y-1"><p className="text-[9px] uppercase text-zinc-500 font-black tracking-widest">Revenue</p><p className="text-2xl font-mono font-bold text-white">${profile.revenueEarned?.toLocaleString()}</p></div>
                <div className="space-y-1"><p className="text-[9px] uppercase text-zinc-500 font-black tracking-widest">Active Hires</p><p className="text-2xl font-mono font-bold text-white">{profile.timesHired}</p></div>
                <div className="space-y-1"><p className="text-[9px] uppercase text-zinc-500 font-black tracking-widest">Avg Concurrent</p><p className="text-2xl font-mono font-bold text-white">{profile.avgAudienceSize?.toLocaleString()}</p></div>
                <div className="space-y-1"><p className="text-[9px] uppercase text-zinc-500 font-black tracking-widest">Niche</p><p className="text-2xl font-bold text-[#BF953F] uppercase tracking-tighter">{profile.niche}</p></div>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 pt-8 border-t border-white/5">
              <div className="space-y-4">
                ...
                <div className="flex gap-1 h-[60px] mono">
                  ...
                  <div className="flex-[3] relative">
                    ...
                    <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-zinc-600"><Icons.ChevronDown className="w-3 h-3"/></div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex gap-4 items-end">
                <div className="flex-1 space-y-4">
                  <label className="text-[9px] uppercase text-zinc-600 font-black tracking-widest">USDC Value</label>
                  <div className="relative h-[60px]">
                    <input 
                      ...
                      required 
                    />
                    <div className="absolute right-5 top-1/2 -translate-y-1/2 text-[9px] font-black text-zinc-800 uppercase tracking-widest pointer-events-none">SETTLEMENT</div>
                  </div>
                </div>
                <button onClick={handleListSubmit} disabled={isListing} className="flex-1 h-[60px] bg-white text-black font-black uppercase text-[11px] tracking-[0.4em] hover:bg-[#BF953F] hover:text-white transition-all shadow-lg">
        ...
        <div className="glass p-12 rounded-none border-white/10 flex flex-col relative overflow-hidden">
          ...
          {sponsorApp?.status === SponsorStatus.APPROVED ? (
            <div className="p-16 text-center space-y-10">
              <Icons.Check className="w-20 h-20 mx-auto text-[#BF953F]" />
