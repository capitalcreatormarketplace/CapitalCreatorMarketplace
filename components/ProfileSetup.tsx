              ...
              <form onSubmit={handleProfileSubmit} className="space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      className="w-full bg-black/50 border border-white/10 p-4 rounded text-base font-bold focus:border-white/40 outline-none"
                      value={formData.name}
                ...
                <div className="space-y-2">
                  ...
                  <textarea 
                    ...
                    className="w-full bg-black/50 border border-white/10 p-4 rounded min-h-[110px] text-[12.2px] focus:border-white/40 outline-none"
                    value={formData.bio}
              ...
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-3 gap-8 py-2">
                  <div className="space-y-1">
                    <p className="text-[7.9px] uppercase text-zinc-500 font-black tracking-widest">Revenue Earned</p>
                    <p className="text-xl font-mono font-bold">${profile.revenueEarned?.toLocaleString()}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-[7.9px] uppercase text-zinc-500 font-black tracking-widest">Times Hired</p>
                    <p className="text-xl font-mono font-bold">{profile.timesHired}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-[7.9px] uppercase text-zinc-500 font-black tracking-widest">Avg Audience</p>
                    <p className="text-xl font-mono font-bold">{profile.avgAudienceSize?.toLocaleString()}</p>
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className={`glass p-7 rounded-xl border-white/10 flex flex-col ${profile.role === UserRole.SPONSOR ? 'opacity-30 pointer-events-none' : ''}`}>
            ...
            <div className="space-y-5 flex-grow">
              <p className="text-[12.2px] text-gray-400">List your available content inventory. Reach premium sponsors directly with no middleman.</p>
              
              <div className="space-y-4 pt-4 border-t border-white/5">
                <div className="space-y-1">
                  <label className="text-[7.9px] uppercase text-gray-500 font-bold">Content Platform</label>
                  <select 
                    className="w-full bg-black border border-white/10 p-2 rounded text-[12.2px]"
                    value={invData.platform}
                    onChange={e => setInvData({...invData, platform: e.target.value as any})}
                  >
                ...
                <div className="space-y-1">
                  <label className="text-[7.9px] uppercase text-gray-500 font-bold">Content Date and Time</label>
                  <input 
                    type="datetime-local" 
                    className="w-full bg-black border border-white/10 p-2 rounded text-[12.2px]"
                ...
                <div className="space-y-1">
                  <label className="text-[7.9px] uppercase text-gray-500 font-bold">Placement Details</label>
                  <textarea 
                    placeholder="e.g. 30 second mid-roll shoutout + pinned link"
                    className="w-full bg-black border border-white/10 p-2 rounded text-[12.2px] h-20"
            ...
            <button 
              ...
              className="mt-8 bg-white text-black w-full py-3.5 font-bold uppercase text-[10.3px] flex items-center justify-center gap-2"
            >
          ...
          <div className={`glass p-7 rounded-xl border-white/10 flex flex-col ${profile.role === UserRole.CREATOR ? 'opacity-30 pointer-events-none' : ''}`}>
            <div className="flex items-center justify-between mb-5">
               <h3 className="text-[0.94rem] font-bold uppercase tracking-tight">Sponsor Portal</h3>
               <span className="bg-white/5 border border-white/10 px-2.5 py-1 text-[7.9px] uppercase">Client</span>
            </div>

            <div className="space-y-5 flex-grow">
              <p className="text-[12.2px] text-gray-400">Apply to become an authorized sponsor. Once approved, you can purchase any listing instantly.</p>
              ...
              {sponsorApp?.status === SponsorStatus.APPROVED ? (
                <div className="p-4 bg-white/10 border border-white/20 rounded-lg flex items-center gap-3">
                  <div className="w-7 h-7 rounded-full bg-white text-black flex items-center justify-center scale-90">
                    <Icons.Check />
                  </div>
                  <div>
                    <p className="text-[11.2px] font-bold uppercase">Account Verified</p>
                    <p className="text-[9.4px] text-gray-400">Access to Marketplace unlocked.</p>
                  </div>
                </div>
              ) : sponsorApp?.status === SponsorStatus.PENDING ? (
                <div className="p-4 bg-white/5 border border-dashed border-white/20 rounded-lg">
                  <p className="text-[11.2px] font-bold uppercase">Application Pending</p>
                  <p className="text-[9.4px] text-gray-400 mt-1">Review typically takes 24-48 hours. Notifications sent to CapitalCreatorMarketplace@gmail.com</p>
                </div>
              ) : (
                <form className="space-y-4 pt-4 border-t border-white/5" onSubmit={(e) => { e.preventDefault(); onApplySponsor(appData); }}>
                  <div className="space-y-1">
                    <label className="text-[7.9px] uppercase text-gray-500 font-bold">Contact Name</label>
                    <input 
                      type="text" 
                      className="w-full bg-black border border-white/10 p-2 rounded text-[12.2px]"
                      value={appData.name}
                  ...
                  <div className="space-y-1">
                    <label className="text-[7.9px] uppercase text-gray-500 font-bold">Company Name</label>
                    <input 
                      type="text" 
                      className="w-full bg-black border border-white/10 p-2 rounded text-[12.2px]"
                  ...
                  <div className="space-y-1">
                    <label className="text-[7.9px] uppercase text-gray-500 font-bold">Months in Business</label>
                    <input 
                      type="number" 
                      className="w-full bg-black border border-white/10 p-2 rounded text-[12.2px]"
                  ...
                  <div className="space-y-1">
                    <label className="text-[7.9px] uppercase text-gray-500 font-bold">Logo URL</label>
                    <input 
                      type="url" 
                      placeholder="https://..."
                      className="w-full bg-black border border-white/10 p-2 rounded text-[12.2px]"
                  ...
                  <button type="submit" className="w-full bg-white text-black py-3.5 mt-3 font-bold uppercase text-[10.3px] tracking-widest">
                    Submit Application
                  </button>
                </form>
              )}
